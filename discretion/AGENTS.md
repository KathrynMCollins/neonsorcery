...
- Django project root lives here; `manage.py` is the entry point and `discretion/` contains settings and core configuration.
...
- Data and utilities are in `contrib/` and project artifacts like `discretion.sql.bz2` are stored at the repo root.
...
- Treat `discretion.sql.bz2` as data backup/reference; do not edit it directly in PRs.
